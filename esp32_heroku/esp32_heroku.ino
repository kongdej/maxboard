...
String webhook = "http://maxboard-php.herokuapp.com/"; // local proxy server for linebot
...
int intensity =  2; // default LED intensity

WebServer server(80);

void wifi( void * pvParamenters) {
    mx.begin(intensity);      // Max7219 begin
  ...
  while(1) {
    ...
    if (t == "clock" || t == "") {
      ...
      if (t.substring(0,1) == ">"){ // pacman
        ...
        printLedScroll("Scroll delay = "+t.substring(1,t.length())+" msec");
      }
      else if (t.substring(0,1) == "*") { // set speed
        intensity = t.substring(1,t.length()).toInt();
        Serial.print("Intensity = ");Serial.println(intensity);
        printLedScroll("Set intensity = "+t.substring(1,t.length()));
      }
      else if (t == "clr") {  // clear
        printLedText(" ");
      }
...
void  getWebhook(void *pvParameters){
  ...
  while(1){
    ...
    Serial.println(webhook + String(boardId) + ".html?" + String(millis()));                                    
    if (httpCode > 0) { 
        t = http.getString();
    }
    else {
...
void accessPoint(void *pvParameters) {
      ...
      String msg = "SSID="+String(ssidap)+" -> 192.168.4.1";
      while(1) {
       printLedScroll(msg);
       vTaskDelay(1000 / portTICK_PERIOD_MS); 
     }
...
void handleList() {
  ...
  content += "Password = xxxxxxxxxxxxxxx<br>";
